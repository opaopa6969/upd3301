...
| `refdrv.cpp` | ヘッドレスdriver: `.d88`起動→フック駆動→状態ダンプ。**イメージ0をドライブ0、イメージ1をドライブ1に挿す**（下記） |
| `PATCHES.md` | 各フックの説明（新しいM88Mへ移植する用） |
| `README.md` | English版 |

M88M本体をリポジトリに入れる必要はない ── `build.sh`が取ってきてパッチを当てる。

### 両側を同じ機械にすること（2026-08-13 に踏んだ穴）

`.d88` は複数イメージを持てる。**353本中202本が複数イメージ**で、Node 側の
`tools/batch-compare.mjs` は前からイメージ1をドライブ1に挿していた。`refdrv.cpp` は
`Mount(0, …)` しか呼んでおらず、**過半数のタイトルで両者が別の機械を走らせていた**。

USER ディスクやディスクBを要求するタイトル（Hydlide3 / Stercru / starclsr / PRO_FAN）は
M88 側で空のドライブ1を見て N88-BASIC のプロンプトに落ち、**5日間「M88 が起動できない
タイトル」として記録されていた**。ドライブ1をマウントしたら4本とも起動した
（完全一致 328→332・追従 333→341・乖離 18→11）。

**refdrv を触るときは、Node ハーネスと同じ枚数・同じ順でディスクを挿しているか確かめること。**
