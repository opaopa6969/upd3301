...
| | QUASI88 | MAME | M88 | **本repo** |
|---|---|---|---|---|
| 画面の出所 | **VRAM直読み**（DMACのch2アドレスを基点に毎フレーム同じ場所を読む） | 8257→dack_w→3301行FIFO | PD8257::RequestRead→CRTC行バッファ | DRQ→DMA pull→行バッファ |
| DMAカウンタ(TC) | **未実装**（レジスタに書くだけで誰も読まない） | 実装 | 実装 | 実装 |
| オートロード(ch3→ch2) | **未実装** | 実装 | 実装 | 実装 |
| **27色技の「機構」** | **原理的に不可能** | **再現される** | **再現される** | **再現される** |
| **27色技の「混色」** | — | **しない**（毎フレームbitmapクリア、残光なし） | 未確認 | **する**（蛍光体が時間積分） |
| アトリビュートペア展開 | 実装 | 実装 | 実装 | 実装 |
| 20ペア/行の上限 | 実装 | 実装 | **無し** | 実装 |
| DMAアンダーラン | **未実装**（statusをクリアするだけ） | **未実装**（TODOに明記） | 実装 | 実装 |
| DMAのラスタ内タイミング | 無し（ウェイト数だけ逆算） | **無し**（TODOに明記：フレーム末に一括転送） | **行単位**（scheduler） | 無し（フレーム粒度） |
| FDD / ゲーム | 動く | 動く | 動く | **動かない** |
| 蛍光体・管の物理 | 無し | 無し（HLSLシェーダは別レイヤ） | 未確認 | **有り** |
