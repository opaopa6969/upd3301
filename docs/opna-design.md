...
1. **Per-game OPNA enablement** 🔴 — `0x8FCE` turned out to be a **config byte loaded from disk** (written during the FDD PPI handshake at 0xFC–0xFF), not a runtime detection result; and *forcing* its low-2 bits to 3 + presenting a timer-capable stand-in at 0xA8–0xAB did **not** make Telenet MUSICBOX emit OPNA writes. Conclusion: enabling the enhanced path is **game-specific RE** (a runtime `IN 0xA9` handshake whose expected value depends on real OPNA status/timer semantics, and/or a setup-menu selection), and it can only be cracked by iterating against an actual minimal OPNA — i.e. it is **implementation phase 0**, not a further black-box probe. Telenet MUSICBOX may itself be an OPN-only player that merely peeks 0xA8 once; a cleaner target (Ys III SB2 build, Ys II Music Gallery) is worth trying first. *[build the minimal OPNA at 0xA8–0xAB — address latch + status + Timer A/B — then boot a SB2 title and iterate the status until it flips to the enhanced score; the game's own 0xA8–0xAB writes then become the implementation reference]*
