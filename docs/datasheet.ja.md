...
> 第2巻 — 周辺チップ（8255 / 765 / 8214 / 8251 / クロック）: [peripherals.ja.md](./peripherals.ja.md)
...
SET INTERRUPT MASK にはもう一つ非公開の副作用がある：VRTC割り込みの
マスク解除がステータスビットを巻き込んでクリアする。データシートは
これについて肯定も否定もしておらず、VE が残るかどうかは実装間で
食い違っている。[design.md](./design.ja.md#未検証の挙動分かっていないこと)
の「未検証の挙動」を見ること。**上の表がこの点を決着させていると読まないこと。**
...
ただし「任意」は「無検査」ではない。`resetEx` は `MAX_EX_ROWS`（1024行）と
`MAX_EX_ROW_BYTES`（1行のDMAバースト64KiB）の外を弾き、非整数・負数・
数値以外の引数はクランプせずそのまま拒否する。ポート経路では `Math.min` が
**シリコンの切り捨てそのもの**だが、ここには「黙って別の画面を返した」責任を
負わせられるチップが無いため。
...
- **長残光のカラー管は実在した**（初版で「実質存在しない」と書いたら、
  実際に使っていた作者からツッコミが入った）。代表例が PC-9801XL などの
  ハイレゾ機用純正カラーディスプレイで、インターレース高解像度表示の
  ちらつきを長残光蛍光体で抑えていた。かわりに動くものが盛大に尾を引く。
  「長残光カラー」プリセットはそういう管の性格を狙ったものと思ってほしい。
