...
|--------|------|--------|-----------|------------|----------------------|
| SIO (8251) | 0 | 00h | 1 | bit 2 | なし — CLOADはRXRDYをポーリング。SIO割り込みは出さない |
| VSYNC | 1 | 02h | 2 | bit 1 | 毎フレーム末、60 Hz |
| RTC | 2 | 04h | 3 | bit 0 | インターバルタイマ、600 Hz |
| SOUND (OPN) | 4 | 08h | 5 | —（実機はport 32h側） | YM2203 タイマA/B溢出（machine88.js、立ち上がりエッジ）— ym2203.js は完全実装。port 44h=ステータス、45h=レジスタデータ（0E/0Fはジョイスティック） |
...
# μPD8251 — USART（カセットロード実装済み・バイト単位）
...
## 本リポジトリでの現状（実装済み・M88準拠のバイト単位）

テープは **M88**（`tapemgr.cpp` + `sio.cpp`）と同じく**バイト単位**でモデル化
する（FSK波形ではない）。`.t88`/`.cas` イメージ（[`tape.js`](../tape.js)）は
MARK / SPACE / DATA セグメントの順序付きリストで、モーターが回っている間
（ポート30h bit 3）再生クロックが進み、DATAバイトをブロックのボーレートで
8251受信レジスタに流し込む（1200で44 T88 tick単位/バイト、600で88。
**1単位 = clockHz/4800 T-state**）。CPUから見えるのは：

- **21h（ステータス）** — `TXRDY|TXE`（05h）に、バイトがラッチされている
  ときは `RXRDY`（02h）。追従できている読み手なら `OE`/`FE` はクリア。
  書き込みは8251のモード/コマンド状態機械を回す（**RxE** / エラーリセット /
  内部リセットを実装 — CLOADに十分）。
- **20h（データ）** — 現在のバイト。読むと RXRDY をクリアし、次のバイトを
  「すぐ」要求する（M88の `SetEvent(event,1)` 相当）。だからブロックは
  ボーレートで這うのではなく読み出し速度で流れる。
- **40h bit 2（キャリア）** — MARKに停まっている間だけ high。（これは
  FSKビットストリームでは**ない**。初期の推測はここでFSKをデコードしようと
  して外した。0C00hのROM逆アセンブル — キャリア待ち→`OUT 21h`モード/コマンド
  →`IN 21h`でRXRDYポーリング→`IN 20h` — とM88ソースの両方が8251バイト経路を示す。）

エンドツーエンドで検証済み：N-BASICで `Time.t88` に対し `CLOAD"TIME"` すると
`Found:TIME` と出て、CPUが20hから引いた3852バイトはテープのDATAセグメントと
1バイト単位で一致、`LIST` は綺麗なBASICに逆トークン化される。知っておくと良い
落とし穴が2つ：N-BASICの**ファイル名は大文字小文字を区別する**（ヘッダは大文字
`TIME` を格納するので `CLOAD"TIME"` と打つ必要がある — マシンは小文字モードで
起動するので名前はシフトで大文字にする）、そしてその後 `Syntax error` になる
のはたいてい**方言違い**（例：N88/ディスクBASICのプログラムをN-BASICで実行）で
あってロード不良ではない — バイトは正確。

- SIO**割り込み**は発生させない — N-BASICのCLOADはRXRDYを*ポーリング*する
  ので、8214のソース0の席は配線されたまま待ちぼうけ（ロードには十分。SIO
  割り込みを欲しがるタイトルなら要配線）。
- 長いテープリーダーやブロック間ギャップは `DUR_CAP` で上限を設け、数秒の
  リーダーでCLOADが這わないようにしている（キャリアは上限内で十分安定。実際の
  データ前MARK（≥1000単位）はそのまま）。
