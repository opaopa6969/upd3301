...
test('kanji1: address latch (0xE8/0xE9) + data read returns rom[addr*2 + port&1]', () => {
  const rom = synthRom();
  const m = new Pc8801Machine({ main: main(), kanji: rom });
  for (const addr of [0x0000, 0x1234, 0x7fff, 0xffff]) {
    m.out(0xe8, addr & 0xff);
    m.out(0xe9, (addr >> 8) & 0xff);
    assert.equal(m.in(0xe8), rom[(addr << 1) & (rom.length - 1)], `e8 @ ${addr}`);
    assert.equal(m.in(0xe9), rom[((addr << 1) | 1) & (rom.length - 1)], `e9 @ ${addr}`);
  }
});
...
test('kanji2: level-2 ROM on its own ports (0xEC/0xED), independent address', () => {
  ...
  assert.equal(m.in(0xe8), rom1[(0x2211 << 1) & (rom1.length - 1)], 'kanji1 unaffected by kanji2 writes');
});
